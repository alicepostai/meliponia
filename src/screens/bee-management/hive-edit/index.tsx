...
import { useHiveEditScreenStyles } from './styles';
import { useHiveEditForm, HiveEditFormValues } from './UseHiveEditForm';
...
const EditSchema = Yup.object().shape({
  ...
  purchaseValue: Yup.string().when('hiveOrigin', {
    is: (origin: HiveOrigin | null) => origin && origin.name === 'Compra',
    then: schema =>
...
const HiveEditScreen = memo(() => {
  const styles = useHiveEditScreenStyles();
  const {
    loading,
    error,
    isSubmitting,
                ...
                <View style={styles.fieldsGroupContainer}>
                  <View style={[styles.fieldWrapper, styles.lastFieldWrapper]}>
                    <UnifiedLocationSelector
                      latitude={
                        values && typeof values.latitude === 'number' ? values.latitude : null
                      }
                      longitude={
                        values && typeof values.longitude === 'number' ? values.longitude : null
                      }
...
HiveEditScreen.displayName = 'HiveEditScreen';
